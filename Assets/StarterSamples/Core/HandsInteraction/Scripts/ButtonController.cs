...
namespace OculusSampleFramework
{
    ...
    public class ButtonController : Interactable
    {
        ...
        [SerializeField] private string[] scenes; // List of scene names
        private int currentSceneIndex = 0; // Current scene index
        ...
        void Update()
        {
            // Check if this button is pressed for scene navigation
            if (CurrentButtonState == InteractableState.ActionState)
            {
                if (gameObject.name == "NextSceneButton")
                {
                    LoadNextScene();
                }
                else if (gameObject.name == "PreviousSceneButton")
                {
                    LoadPreviousScene();
                }
            }
        ...
        {
            // Increment the scene index and load the next scene
        ...
        {
            // Decrement the scene index and load the previous scene
